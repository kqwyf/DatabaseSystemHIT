...
- 数据交互：
  ...
  - 更新记录：
    ...
    - update 表名 set 变量名=值 where current of 游标名;
...
- 插入异常：本应暂时允许含空记录的表，含空记录无法插入。
- 删除异常：本应作为独立概念的信息依附于某些实例存在，导致这些实例删除后，前述概念消失。
...
**多值依赖**：对于$X$给定值，$Y$有一组值与之对应，且在确定该$X$值时，对该组$Y$值中的任一个值，与其对应的元组集在$U-X-Y$上的投影与$Y$无关。记作$X\rightarrow \rightarrow Y$。其中$X$与$Y$可以相交。函数依赖是多值依赖的特例。
...
- **保持依赖分解**：对关系模式$R(U,F)$，称$\rho$为$R$相对于$F$的一个保持依赖分解，当且仅当$\forall f\in F,(\mathop \bigcup_{i=1}^k \pi_{R_i}(F))\models f$，其中$\pi_{R_i}(F)=\{X\rightarrow Y\in F|X,Y\in R_i\}$。
  ...
  - 检验算法：
    - 一般情形：设$G=\mathop \cup \limits_{i=1}^k \pi_{R_i}(F)$
...
- 保持依赖地分解为3NF：
  ...
  2. 对于每个在依赖左端出现的属性集$X$，寻找所有$\{X\rightarrow A_1,X\rightarrow A_2,\dots,X\rightarrow A_m\}\subset F$，并将子模式$s(X,A_1,A_2,\dots,A_m)$加入子模式集。
- 无损连接且保持依赖地分解为3NF：
...
- 连接操作：$R\mathop \Join \limits_{R.A \space \theta \space S.B} S$
  ...
  - 大关系实现
    ...
    ```cpp
    for(i = 1; i <= B_S / (M - 2); i++) {
        read_n_th_subset_of(i, S); // 将R_S分成大小为M-2的子集，每轮循环使用其中一个子集
        for(j = 1; j <= B_R; j++) {
...
- 不能重复读：事务自身未改变数据，但前后两次读取间另一事务对数据写入，导致两次读取不一致。
...
可串行性：若对数据库的任意初始状态，一个调度对数据库状态的影响都与某个串行调度相同，则称该调度是可串行化的。正确的并行调度不一定是可串行化的，因为“正确的并行调度”指结果正确，而可串行性要求对数据库影响相同。
...
- 证明：归纳法。设现需调度$n$个事务，其中事务$T_i$在所有事务中具有最早的解锁动作$U_i(X)$。
  ...
  假设协议对除$T_i$外的其余$n-1$个事务可产生冲突可串行化调度，希望$T_i$能被无冲突地移至最前。
...
调度器维护START，VAL，FIN集合记录每个事务的开始时间、确认时间与结束时间。调度器也维护每个事务的读集合与写集合。

对事务$T$进行有效性确认的策略：对$T$开始后进行过写的事务，检查其写集合与$T$的读集合是否有交集，有则不确认。对$T$确认时仍未结束的事务，检查其写集合与$T$的写集合是否有交集，有则不确认。上述两情况都未出现时，确认$T$，否则回滚$T$。注意前述需检查的事务均应为**已确认的事务**。
