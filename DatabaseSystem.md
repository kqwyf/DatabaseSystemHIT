...
- **Create**
  ...
  - create table 表名(列名 数据类型 \[Primary key|Unique\] \[Not null\] \[\[, 列名 数据类型 \[Not null\]\] ...\]);
    ...
    创建指定名称的表，包含指定的属性列，每个属性具有指定的数据类型及约束。
...
- **Alter**
  ...
  - alter table 表名 \[add 列名 数据类型\[\[, 列名 数据类型\] ...\]\] \[drop 列名|完整性约束名(列名)\] \[modify 列名 数据类型\[\[, 列名 数据类型\] ...\]\]

    修改指定表的定义。add用于增加新列，drop用于删除列或完整性约束，modify用于修改列的定义。
...
- **Select**
  ...
  - select \[Distinct\] 列名\[\[ as\] 别名\] \[\[, 列名\[\[ as\] 别名\]\] ...\] from 表名\[\[ as\] 别名\] \[\[, 表名\[\[ as\] 别名\]\] ...\] \[where 检索条件\] \[order by 列名 \[asc|desc\]\];

    从表名所给出的表（的笛卡尔积）中，查询满足检索条件的元组，并按照指定的列及其顺序投影显示。相当于$\pi_{列名,\dots, 列名}(\sigma_{检索条件}(表名))$。列名可用“*”表示所有列。检索条件中可使用运算符and、or、not及括号，还可使用模糊匹配运算like或not like，属于运算in；多个表中属性名相同时，以“表名.属性名”方式指定属性。
