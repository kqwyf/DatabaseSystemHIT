...
**插入操作**：从根至下搜索待插入位置，然后从底至上插入并进行节点分裂，直至所更新节点无需分裂为止。分裂时将节点内指针均分（多者归右节点）。当最终更新至无需分裂的非叶节点时，非叶节点中将增加一个索引字段值，其由最后一次分裂的右节点的左边界值决定（由右节点向下查询至叶节点即可确定）。
